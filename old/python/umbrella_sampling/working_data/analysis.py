...
def calculate_height_dist(data_array):
    ...
    #print z_window_edges
    #print z_windows
    #quit()
    import matplotlib.pyplot as plt
    for j in range(num_windows):
        binsPerWindow= num_bins/num_windows
        rng = (float(j)*total_frames/num_windows,(j+1.)*total_frames/num_windows)
        rng = (int(rng[0]),int(rng[1]))
        binContents[j],bins[j] = np.histogram(Z[rng[0]:rng[1]+1],bins=binsPerWindow,range=z_windows[j])
        #print bins[j]
        #plt.hist(Z,bins=binsPerWindow-1,range=z_windows[j],histtype='bar')
    #plt.show()
    #quit()
    # this should restore detailed balance
    ...
    import math
    for j in range(num_windows):
        for k in range(binsPerWindow):
            if binContents[j][k] != 0:
                Z.append(bins[j][k])
                A.append(-math.log(binContents[j][k]))
    print Z
    plt.plot(Z,A)
    plt.show()
    quit()
