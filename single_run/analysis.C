void analysis() {
	...
	tpG->Draw("AC*");
	tpFunc->Draw("same");
	//tpG->GetHistogram()->SetTitle("probability dist with Gaussian curve");
	//tpFunc->Draw();
	...
	rpG->Draw("AC*");
	rpFunc->Draw("same");
	//rpG->GetHistogram()->SetTitle("probability dist w/ Gaussian curve");
	//rpFunc->Draw();
	...
	rptpG->Draw("AC*");
	rptpFunc->Draw("same");
	//rptpG->GetHistogram()->S("probability dist w/ Gaussian curve");
	//rptpFunc->Draw();
	...
	dcosG->Draw("AC*");
	cosFunc->Draw("same");
	//dcosG->GetHistogram()->SetTitle("probability dist w/ theoretical curve");
	//cosFunc->Draw();

	//c->Update();
	c->Print("results_1e9_events.pdf");
	//c->Modified();
