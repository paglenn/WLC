...
N_max = 40000              # maximum number of snapshots/simulation
val_min = 90                 # min for PMF
...
for k in range(K):
    ...
    line = lines[k]
    tokens = line.split()
    val0_k[k] = float(tokens[1]) # spring center location
    K_k[k] = float(tokens[2]) # spring constant
    trajf_k.append(str(tokens[0])) # infile name
    #trajf_k.append("window_N=" + tokens[0] + ".txt")
    #if len(tokens) > 3:
    #    T_k[k] = float(tokens[4])  # temperature the kth simulation was run at.
...
# Initialize MBAR.
print "Running MBAR..."
#mbar = pymbar.MBAR(u_kln, N_k, verbose = True, method = 'adaptive', initialize='BAR', relative_tolerance=1.0e-1)
