// for simple sampling of the WLC 
// Paul Glenn
// input delta (discretization) 
void analysis() {
	double delta = 0.001; 
	double PI = acos(-1);
	TCanvas *c = new TCanvas("Analysis","1e7events");
	c->Divide(2,2);
	...
	std::cout<<"tp2 avg: " << tp_var + pow(tp_mu,2)<< std::endl; 
	...
	std::cout<<"rp2 avg: " << rp_var +pow(rp_mu,2) << std::endl; 
	...
	std::cout<<"rptp avg: " << rptp_mu << std::endl; 
