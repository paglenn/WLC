...
double deltaE(int index, int w_index, double z_prev ) {
	...
	if( index+1 != N) {
		E_old -= kappa*dot(tx[index+1],ty[index+1],tz[index+1],t_old[0],t_old[1],t_old[2]) ;
		E_new -= kappa*dot(tx[index+1],ty[index+1],tz[index+1],tx[index],ty[index],tz[index]) ;
	}
...
int cleanup() {
	...
	sumCos -> Scale(1./sumCos->Integral("width")) ;
	sumCos->Write("sum_cos_eq_rp");
